...
async def fetch_news_from_rss(rss_urls: List[str]) -> List[Dict]:
    ...
    # Optimized RSS feeds - Focused on requested categories
    if not rss_urls:
        rss_urls = [
            # === ECONOMÍA ===
            ...
            "https://www.eleconomista.com.mx/rss/economia.xml",  # El Economista MX
            # === NOTICIAS GENERALES ===
            ...
            "https://feeds.skift.com/skift",  # Skift Travel Industry
        ]
    ...
    try:
        ...
        print(
            f"📊 RSS Summary: {successful_feeds} successful, {failed_feeds} failed, {len(news_items)} total items"
        )
...
async def fetch_single_feed(url: str) -> List[Dict]:
    ...
    try:
        ...
        if hasattr(feed, "bozo") and feed.bozo:
            print(
                f"⚠️ Feed parsing warning for {url}: {getattr(feed, 'bozo_exception', 'Unknown error')}"
            )
