...
# Import services
from services.tasks_local import (
    list_today_sorted,
    add_task,
    add_recurrent_task,
    complete_task,
...
def setup_google_credentials_for_render():
    ...
    try:
        ...
        print(
            "📋 Make sure CREDENTIALS_JSON_BASE64 and TOKEN_JSON_BASE64 are set in Render"
        )
...
def get_task_inline_keyboard(task_id: str) -> InlineKeyboardMarkup:
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "Marcar como hecha ✅", callback_data=f"complete_{task_id}"
            )
        ]
    ]
    ...
    try:
        ...
        if local_tasks:
            ...
            for task in local_tasks[
                :3
            ]:  # Limit to first 3 tasks to avoid too many buttons
                buttons.append(
                    [
                        InlineKeyboardButton(
        ...
        await update.message.reply_text(
            "❌ Error al obtener las tareas. Intenta de nuevo."
        )
...
async def cmd_recur(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await update.message.reply_text(
            "❌ Uso: /recur <descripción de la tarea recurrente>"
        )
        return
    ...
    try:
        ...
        await update.message.reply_text(
            "❌ Error al crear la tarea recurrente. Intenta de nuevo."
        )
...
async def cmd_done(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if success:
            ...
            await update.message.reply_text(
                f"❌ No se encontró la tarea con ID `{task_id}`"
            )

    except Exception as e:
        logger.error(f"Error in cmd_done: {e}")
        await update.message.reply_text(
            "❌ Error al completar la tarea. Intenta de nuevo."
        )
...
async def cmd_ia(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if intent == "clarify":
            ...
            task_id = add_task(title, notes, priority, due)
            await update.message.reply_text(
                f"✅ Tarea creada: {title} (ID: `{task_id}`)", parse_mode="Markdown"
            )
            ...
            await update.message.reply_text(
                f"🔄 Tarea recurrente creada: {title} (ID: `{task_id}`)",
                parse_mode="Markdown",
            )
            ...
            if instruction:
                ...
                await update.message.reply_text(
                    "❌ No se pudo procesar la instrucción de preferencias"
                )

        else:
            await update.message.reply_text(
                f"🤖 Intent detectado: {intent}, pero no implementado aún"
            )

    except Exception as e:
        logger.error(f"Error in cmd_ia: {e}")
        await update.message.reply_text(
            "❌ Error al procesar la instrucción. Intenta de nuevo."
        )
...
async def generate_brief_background(
    ...
    try:
        ...
        emails_data = {
            ...
            "selected": 0,
            "emails": [],
            "rationale": "📧 Emails no disponibles",
        }
        calendar_data = []
        tasks_data = []
...
async def fetch_and_rank_emails() -> Dict:
    ...
    try:
        ...
        logger.info(
            f"🔍 Filtered to {len(filtered_emails)} emails from {original_count} total"
        )
        ...
        try:
            ...
            logger.info(
                f"✅ Email ranking completed, selected {ranked_result.get('selected', 0)} emails"
            )
        except Exception as ai_error:
...
async def show_ai_config_menu(
    ...
    try:
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    f"{gemini_status} Gemini", callback_data="ai_provider_gemini"
                ),
                InlineKeyboardButton(
                    f"{openrouter_status} OpenRouter",
                    callback_data="ai_provider_openrouter",
                ),
            ]
        )
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    f"📋 Actual: {current_provider.title()} ({current_model})",
                    callback_data="ai_config_info",
                )
            ]
        )
...
async def handle_provider_selection(
    ...
    try:
        ...
        # Back button
        keyboard.append(
            [InlineKeyboardButton("⬅️ Volver", callback_data="ai_config_back")]
        )
...
async def handle_model_selection(
    update: Update, context: ContextTypes.DEFAULT_TYPE, provider: str, model: str
) -> None:
    ...
    try:
        ...
        keyboard = [
            [InlineKeyboardButton("⬅️ Volver al menú", callback_data="ai_config_back")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
# Button handlers
async def handle_button_message(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
...
async def main() -> None:
    ...
    try:
        ...
        if os.path.exists(lock_file):
            logger.warning(
                "⚠️ Lock file exists - checking if another instance is running"
            )
            try:
        ...
        application.add_handler(
            MessageHandler(filters.TEXT & ~filters.COMMAND, handle_button_message)
        )
        ...
        try:
            ...
            if "Conflict" in str(polling_error):
                logger.warning(
                    "⚠️ Bot conflict detected - another instance may be running"
                )
                ...
                try:
                    ...
                    logger.error(
                        f"❌ Failed to start polling after retry: {retry_error}"
                    )
                    raise
            else:
                raise polling_error
